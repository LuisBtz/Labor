...
import PostPublicationEn from './posts/PostPublicationEn';
  ...
  return (
    <PublicationContainer onMouseMove={() => setMove(move)}>
      ...
      {data.sanityHomePage.publicationHighlights.map(({ title, date, abstract, author, slug, metaData, id, thumbnailCover }) => {
        return (
          <PostPublicationEn title={title} thumbnailCover={thumbnailCover} slug={slug} date={date} metaData={metaData} author={author} key={id} abstract={abstract} />
        )
      })}
