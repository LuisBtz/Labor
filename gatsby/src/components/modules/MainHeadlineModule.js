...
const MainHeadlineModule = ({data}) => {
  ...
  return (
    ...
    <MainHeadlineWrapper>
      ...
      <h1>{text}</h1>
      <Buttons>
        {data.buttons.es?.agenda === true &&
          <li><Link to='/contact' className="cta small caps">Agenda tu cita</Link></li>
        }

        {data.buttons.es?.dossier &&
          <li><a rel="noreferrer" target='_blank' href={data.buttons.es.dossier}>Dossier</a></li>
        }

        {data.buttons.es?.resume &&
          <li><a rel="noreferrer" target='_blank' href={data.buttons.es.resume}>Resume</a></li>
        }
